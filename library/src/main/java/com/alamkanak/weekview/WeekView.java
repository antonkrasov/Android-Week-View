...
public class WeekView extends View {
    ...
    private void init() {
        ...
        Rect rect = new Rect();
        mTimeTextPaint.getTextBounds("00:00 PM", 0, "00:00 PM".length(), rect);
        mTimeTextWidth = mTimeTextPaint.measureText("00:00 PM");
        ...
        mHeaderTextPaint.getTextBounds("00:00 PM", 0, "00:00 PM".length(), rect);
...
//        canvas.drawRect(0, 0, mHeaderColumnWidth, getHeight(), mHeaderColumnBackgroundPaint);
        ...
        boolean _30min = false;
        for (int i = 0; i < 48; i++) {
//            float top = mHeaderTextHeight + mHeaderRowPadding * 2 + mCurrentOrigin.y + mHourHeight * i + mHeaderMarginBottom;
            float top = mCurrentOrigin.y + mHourHeight * i;

            // Draw the text if its y position is not outside of the visible area. The pivot point of the text is the point at the bottom-right corner.
            String time = getDateTimeInterpreter().interpretTime(i / 2, _30min ? 30 : 00);
            if (time == null)
...
//                canvas.drawRect(start, mHeaderTextHeight + mHeaderRowPadding * 2 + mTimeTextHeight/2 + mHeaderMarginBottom, startPixel + mWidthPerDay, getHeight(), sameDay ? mTodayBackgroundPaint : mDayBackgroundPaint);
                canvas.drawRect(start, 0, startPixel + mWidthPerDay, getHeight(), sameDay ? mTodayBackgroundPaint : mDayBackgroundPaint);
...
//                    float top = mHourHeight * 24 * mEventRects.get(i).top / 1440 + mCurrentOrigin.y + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 + mEventMarginVertical;
                    float top = mHourHeight * 48 * mEventRects.get(i).top / 1440 + mCurrentOrigin.y + mTimeTextHeight / 2 + mEventMarginVertical;
...
//                    bottom = mHourHeight * 24 * bottom / 1440 + mCurrentOrigin.y + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - mEventMarginVertical;
                    bottom = mHourHeight * 48 * bottom / 1440 + mCurrentOrigin.y + mTimeTextHeight / 2 - mEventMarginVertical;
